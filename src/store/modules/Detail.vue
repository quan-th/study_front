      ...
      <div class="text-center">
        <v-dialog v-model="dialog" persistent width="500" style="display:unset;">
          <v-card>
            ...
            <v-card-text>Bạn muốn xóa Customer này?</v-card-text>
            <v-divider></v-divider>
...
export default {
  ...
  methods: {
    ...
    handleDelete: function() {
      ...
      ApiService.delete(`customer/` + this.$props.customerCode)
        .then(() => {
          this.$router.push({ name: "home" });
        })
        .catch(e => {
  ...
  mounted() {
    ...
    ApiService.setHeader();
    ApiService.get(`customer/` + this.$props.customerCode)
      .then(response => {
